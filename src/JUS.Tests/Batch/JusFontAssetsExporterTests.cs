...
[TestFixture]
public class JusFontAssetsExporterTests
{
    [Test]
    public void ExportedContainerAreNotDisposed()
    {
        ...
        using (NitroRom testRoot = TestDataBase.ReadSoftware()) {
            actual = new JusFontAssetsExporter().Convert(testRoot);
        }
    ...
    public Task VerifyExportedContainer()
    {
        ...
        NodeContainerFormat actual = new JusFontAssetsExporter().Convert(root);
